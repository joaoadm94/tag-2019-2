...
void imprimirVertices(tVertice *vertice) {
    ...
    if (vertice == NULL) {
        ...
        while (flagVertice) {
            ...
            vertice->grau = grau;
            printf("Disciplina %s, %d creditos\n", vertice->nome, vertice->peso);
...
// Realiza ordenacao topologica por busca em profundidade.
...
int converteCaminhoCriticoParaArquivoVisual(){
    ...
    FILE *pArq; 
    tNoOrdTop *noOrdTopAux = listaOrdTop.inicio;
    tVertice *tmpVerticeCritico = maiorESegundoMaiorCaminho.Maior;
    // int i;
...
int main() {
    ...
    zeraListaOrdTop();
    printf("\nRealizando ordenacao topologica do grafo... \n");
    ordenacaoTopologica();
